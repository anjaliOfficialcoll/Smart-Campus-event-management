...
| Event Details | Full event info, feedback, ratings, and venue map |
...
| Email Confirmation | Receive a confirmation email with QR entry pass after registration |
| QR Entry Pass | Unique QR code generated per registration for event check-in |
| Venue Map | Google Maps embed on event details page showing venue location |
...
## 📧 Email Confirmation & QR Code Entry Pass

After a student successfully registers for an event, the system automatically:
1. Generates a unique QR code containing the student's registration details
2. Sends a confirmation email to the student's email address with:
   - Full event details (name, date, time, venue, type, department)
   - The QR code embedded as an entry pass image
...
> **Note:** Email sending is asynchronous — it does not block or affect the registration process if it fails.

### QR Code API

You can also generate a QR code directly via the REST API:

```
GET /api/qr?studentName=Arjun&studentId=CS001&eventName=Workshop&eventDate=25+Apr+2026&registrationId=1
```
...
## 🗺️ Google Maps Venue Location

Each event details page now shows an embedded Google Maps view of the venue location. The map is loaded using the venue name as the search query — no API key required for basic embed usage.

To use the full Maps JavaScript API with an API key, update `application.properties`:
